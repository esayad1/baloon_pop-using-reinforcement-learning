...
def get_reward(action):
    ...
    # L'agent a fait une action inutile (il a joué deux fois de suite la même action)
    if current_step_tmp == current_step:
        if debug:
            print(f"Récompense: -10 | L'agent a fait une action inutile")
        return -10
    
    # On insite l'agent à sauvegarder ses checkpoints tous les 3 tours (pour éviter de les perdre, on joue sécurisé)
    elif (current_step - 1)%3 == 0:
        if action == 1:
            if debug:
...
def reset():
    ...
    Réinitialise le jeu.
    """
    global current_step, current_episode, current_player, opponent_player, dices, dices_rolls, board, player_one, player_two, nb_victories, loss,current_step_tmp
...
def get_current_player_possible_actions():
    ...
    if len(current_player.checkpoints_columns) >= 3:
        # On retire les actions possible qui ne sont pas dans parmis les colonnes en cours
        possible_actions = [action for action in possible_actions if action in [int(column) for column in current_player.checkpoints_columns]]
...
def step(action):
    ...
    """
    global current_step, dices, dices_rolls, board, current_player, opponent_player, debug, current_step_tmp
    ...
    # Si l'on souhaite joué une colonne    
        ...
        if int(action) not in get_current_player_possible_actions():
            if debug:
                print(f"{current_player.name} ne peut pas jouer cette colonne : {int(action)} | {get_current_player_possible_actions()} | {action in get_current_player_possible_actions()}")
            return
        
        if action not in current_player.checkpoints_columns and action not in current_player.checkpoints_columns_finished and int(action) in get_current_player_possible_actions() and len(current_player.checkpoints_columns) < 3:
            if debug:
                print(f"{current_player.name} ajoute la colonne {action} à ses colonnes en cours")
        ...
        if action in current_player.checkpoints_columns and current_player.checkpoints[action] < board_limits[action]:
            if debug:
                print(f"{current_player.name} avance sur la colonne {action}")
        ...
        if current_player.checkpoints[action] == board_limits[action] and not action in current_player.checkpoints_columns_finished:
            if debug:
                print(f"{current_player.name} a terminé la colonne {action}")
...
current_episode = 1
current_step = 1
current_step_tmp = 0 # Sauvegarde le tour précédent pour savoir si l'agent à fait une action qui fait avancer le jeu
...
def step_QLearning():
    ...
    # Récupération de la récompense pour l'action réalisée
...
def step_DDQN():
    ...
    # On met à jour les poids du réseau cible tous les 20 épisodes
...
def step_Reinforce():
    ...
    possible_actions = get_current_player_possible_actions()
...
if __name__ == '__main__':
   ...
   while True:
        ...
        if len(current_player.checkpoints_columns) == 3 and not any([int(column) in get_current_player_possible_actions() for column in current_player.checkpoints_columns]):
            if debug:
                print(f"{current_player.name} ne peut pas jouer | {get_current_player_possible_actions()} | {current_player.checkpoints_columns}")
